...
failed_app=${rollback_dir}/failed-after-rollback-ChatGPT-Math-26.818.41509-History.app
...
[[ -d ${old_math_app} ]] || { print -u2 'BLOCKED_ROLLBACK: old math app missing'; exit 1; }
...
[[ -n ${manifest_old_bundle_id} && -n ${manifest_old_version} && ${current_old_bundle_id} == ${manifest_old_bundle_id} && ${current_old_version} == ${manifest_old_version} ]] || {
  print -u2 'BLOCKED_ROLLBACK: old math copy no longer matches deployment manifest'
  exit 1
}
